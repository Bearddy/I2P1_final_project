...
static void ghost_move_script_FREEDOM_random(Ghost* ghost, Map* M) {
	...
	int rand = generateRandomNumber(0, cnt - 1);
	...
	int cnt = 0;
	for (Directions i = 1; i <= 4; i++)
		if (i != counter_one && ghost_movable(ghost, M, i, false))
			proba[cnt++] = i;
	/*game_log("\n------------------------------");
	game_log("cnt : %d", cnt);
	game_log("counter : %d", counter_one);
	for (int i = 0; i < cnt; i++) {
		game_log("directions : %d", proba[i]);
	}
	game_log("---------------------------");
	*/
	if (cnt >= 1) {
...
static void ghost_move_script_GO_OUT(Ghost* ghost, Map* M) {
	...
	if(M->map[ghost->objData.Coord.y][ghost->objData.Coord.x] == 'B' || M->map[ghost->objData.Coord.y][ghost->objData.Coord.x] == 'G')
		ghost_NextMove(ghost, UP);
	else
...
static void ghost_move_script_FLEE(Ghost* ghost, Map* M, const Pacman * const pacman) {
	...
	static Directions proba[4];
	for (Directions i = 1; i <= 4; i++) {
		if(i != shortestDirection && ghost_movable(ghost, M, i, false)){
			proba[cnt++] = i;
		}
	}
	if (cnt >= 1) {
