...
void ghost_draw(Ghost* ghost) {
	...
	if (ghost->status == FLEE) {
		...
		*/ 
		//game_log("power up timer: %d, power_up_dur : %d", al_get_timer_count(get_power_up_timer()), get_power_up_dur());
		if (al_get_timer_count(get_power_up_timer()) > 0.7 * get_power_up_dur()) {
			//game_log("test complete");
			if((ghost->objData.moveCD >> 4)& 1 == 1){
				bitmap_x_offset = 32;
			}
			else {
				bitmap_x_offset = 0;
...
bool ghost_movable(const Ghost* ghost, const Map* M, Directions targetDirec, bool disallow_room) {
	...
	switch (targetDirec) {
	case UP:
		if (is_wall_block(M, x, y - 1) || (disallow_room && is_room_block(M, x, y - 1)) || ((ghost->status == FREEDOM || ghost->status == FLEE) && is_room_block(M, x, y-1)))
			return false;
		break;
	case DOWN:
		if (is_wall_block(M, x, y + 1) || (disallow_room && is_room_block(M, x, y + 1)) || ((ghost->status == FREEDOM || ghost->status == FLEE) && is_room_block(M, x, y + 1)))
		...
		break;
	case LEFT:
		if (is_wall_block(M, x - 1, y) || (disallow_room && is_room_block(M, x - 1, y)) || ((ghost->status == FREEDOM || ghost->status == FLEE) && is_room_block(M, x-1, y)))
			return false;
		break;
	case RIGHT:
		if (is_wall_block(M, x + 1, y) || (disallow_room && is_room_block(M, x + 1, y)) || ((ghost->status == FREEDOM || ghost->status == FLEE) && is_room_block(M, x+1, y)))
