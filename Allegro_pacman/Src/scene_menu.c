...
static char* map_name_pos ;
static char map_name[50] = { '\0' };

char my_bgm_name[4][100] = { "Assets/Music/original_theme.ogg", "Assets/Music/silent_hill_theme.ogg", "Assets/Music/idiot_theme.ogg" };
...
static void draw() {
	...
	al_draw_text(
		...
		SCREEN_H - 290,
		ALLEGRO_ALIGN_CENTER,
		"Arrow_left/Arrow_right to change the map"
	);
	

	memset(text, '\0', sizeof(text));
	memset(map_name, '\0', sizeof(map_name));
	map_name_pos = return_map_names(return_map_index());
	strncpy(map_name, &map_name_pos[7], strlen(map_name_pos) - 7 - 4);
	strcpy(text, "Selected Map : ");
	strcat(text, map_name);
	al_draw_text(
		menuFont,
		al_map_rgb(0, 255, 50),
		SCREEN_W / 2,
...
//------------------------------------my code------------------------------------
/*
static void change_to_next(void) {
...
static void change_to_prev(void) {
